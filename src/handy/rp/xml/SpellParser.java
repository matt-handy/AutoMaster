...
public class SpellParser {
	...
	public static Spell load(String filename, boolean actionSpell) throws Exception {
		...
		if(healingList != null && healingList.item(0) != null) {
			...
			DICE_TYPE dice = DICE_TYPE.getDice(healingListElement.getElementsByTagName("dice").item(0).getTextContent());
			int diceCount = Integer.parseInt(healingListElement.getElementsByTagName("diceCount").item(0).getTextContent());
			...
			String extraDicePerLevel = healingListElement.getElementsByTagName("extraDicePerLevel").item(0).getTextContent();
	...
	private static void getDamage(Element parentElement, String damageTagName, Map<SLOTLEVEL, List<SpellDamageComponent>> damagers, int minLevel) {
