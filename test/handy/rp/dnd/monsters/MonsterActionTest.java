		...
		try {
			...
			for(MonsterTemplate mt : allLoaded) {
				if(mt.humanReadableName.contentEquals("Death Knight")) {
					...
					OutcomeNotification result = dk.expendAction("nonaction");
					assertEquals("No such action: nonaction", result.humanMessage);
					assertFalse(result.outcome);
					
					result = dk.expendAction("hellfire_orb");
					assertTrue(result.outcome);
					assertTrue(result.humanMessage.contains("Spell Save: 18"));
					assertTrue(result.humanMessage.contains("Fire damage"));
					assertTrue(result.humanMessage.contains("Hellfire Orb: The death knight hurls a magical ball of fire that explodes at a point it can see within 120 feet of it. Each creature in a 20-foot-radius sphere centered on that point must make a DC 18 Dexterity saving throw. The sphere spreads around corners. A creature takes 35 (10d6) fire damage and 35 (10d6) necrotic damage on a failed save, or half as much damage on a successful one."));
					result = dk.expendAction("hellfire_orb");
					assertEquals("Cannot take action, already acted this turn", result.humanMessage);
					dk.notifyNewTurn();
					result = dk.expendAction("hellfire_orb");
					assertEquals("No remaining charges for action", result.humanMessage);
					...
					assertTrue(result.outcome);
					assertTrue(result.humanMessage.contains("Fire Breath: The dragon exhales fire in a 30-foot cone. Each creature in that area must make a DC 17 Dexterity saving throw, taking 56 (16d6) fire damage on a failed save, or half as much damage on a successful one."));
					dk.notifyNewTurn();
					result = dk.expendAction("fire_breath");
					assertFalse(result.outcome);
					assertEquals("Rechargable Action - Need 5 or better from D6", result.humanMessage);
				}
			}
